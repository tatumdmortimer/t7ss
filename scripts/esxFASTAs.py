...
def get_args():
    ...
    parser.add_argument("esx1", help="ESX-1 genes file", action=FullPaths,
        type=is_file)
    parser.add_argument("esx2", help="ESX-2 genes file", action=FullPaths,
        type=is_file)
    parser.add_argument("esx3", help="ESX-3 genes file", action=FullPaths,
        type=is_file)
    parser.add_argument("esx4", help="ESX-4 genes file", action=FullPaths,
        type=is_file)
    parser.add_argument("esx5", help="ESX-5 genes file", action=FullPaths,
        type=is_file)
...
def make_unaligned_fasta(dnaDirectory, protDirectory, geneList, geneName):
    ...
    for s in species:
        if s == '':
            s = "Mycobacterium_tuberculosis_H37Rv"
...
for f in [args.esx1, args.esx2, args.esx3, args.esx5]:
    with open(f, 'r') as infile:
        for i, line in enumerate(infile):
            if i >= 0:
                ...
                eccE.append(line[5])
                mycP.append(line[6])
with open(args.esx4, 'r') as infile:
    for i, line in enumerate(infile):
        line = line.strip().split()
        eccB.append(line[1])
        eccC.append(line[2])
